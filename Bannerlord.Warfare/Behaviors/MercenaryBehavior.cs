...
namespace Warfare.Behaviors
{
    internal sealed class MercenaryBehavior : CampaignBehaviorBase
    {
        ...
        public void RecruitTroops(Clan clan, bool newGame = false)
        {
            ...
            {
                IEnumerable<Hero> heroes = clan.WarPartyComponents.Where(x => x.PartyOwner.IsAlive && x.MobileParty != null && x.MobileParty.IsActive && x.MobileParty.MapEvent == null && x.MobileParty.SiegeEvent == null).OrderBy(x => x.Party.NumberOfAllMembers).Select(x => x.PartyOwner);
                if (heroes.Count() < 1)
                {
                    queue.Add(clan);
                    return;
