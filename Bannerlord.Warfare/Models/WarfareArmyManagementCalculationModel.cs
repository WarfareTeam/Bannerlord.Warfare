...
namespace Warfare.Models
{
    public class WarfareArmyManagementCalculationModel : ArmyManagementCalculationModel
    {
        ...
        {
            if (party.ActualClan != null && !party.ActualClan.IsEliminated && party.ActualClan.IsMinorFaction && party != MobileParty.MainParty && armyLeaderParty.LeaderHero == armyLeaderParty.MapFaction.Leader)
            {
                return 0;
            }
        ...
        public override ExplainedNumber CalculateDailyCohesionChange(Army army, bool includeDescriptions = false) => _model.CalculateDailyCohesionChange(army, includeDescriptions);
